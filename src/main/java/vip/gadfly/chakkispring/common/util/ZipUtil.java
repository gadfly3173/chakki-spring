...
public class ZipUtil {
     ...
     * @throws IOException IO错误时抛出
     */
    public static boolean zipFile(File resFile, String rootPath, ZipOutputStream zos, String comment, String filename)
            throws IOException {
        rootPath = rootPath + (!StringUtils.hasText(rootPath) ? "" : File.separator) + (StringUtils.hasText(filename) ? filename : resFile.getName());
        if (resFile.isDirectory()) {
            ...
            if (fileList == null || fileList.length <= 0) {
                ...
                for (File file : fileList) {
                    // 如果递归返回false则返回false
                    if (!zipFile(file, rootPath, zos, comment, null)) return false;
                }
            }
        } else {
            ...
            if (!StringUtils.hasText(keyword)) {
                while (entries.hasMoreElements()) {
                    ...
                    if (entryName.contains("../")) {
                        log.error("ZipUtils::" + "entryName: " + entryName + " is dangerous!");
                        continue;
                    }
